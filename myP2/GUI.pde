...
void keyPressed()  // executed each time a key is pressed: sets the Boolean "keyPressed" till it is released   
    ...
    if(key=='k') ;   
    if(key=='l') {
      loadPieces("data/ghost", R_ghost); 
      loadPieces("data/poly", R2); 
      loadPieces("data/poly", R_redo);
      loaded = true;
    }
    if(key=='m') ;
    if(key=='n') ;
    ...
    if(key=='A') {
      savePieces("data/poly");
      int temp = loadCounter("data/poly");
      saveCounter("data/poly", 1);
      loadPieces2("data/orig", "data/poly");
      saveCounter("data/poly", temp);
    }
    if(key=='B') ; 
    if(key=='C') ; 
    if(key=='D') ;  
...
void mousePressed()   // executed when the mouse is pressed
  {
    if (startGame !=2) {
      ...
      if (keyPressed && key=='s' && !moved) {
        if (mouseCount < 1) {P.savePts("data/orig");}
        mouseCount++;
        A=Mouse(); B=Mouse();
      } 
      if (keyPressed && key=='m') 
      ...
      if(keyPressed && key=='q') {
        pt M = Mouse();
        for (pts poly : R2) {
          poly.closestLineOf(M);
        }
        for (pts poly : R_ghost) {
          poly.closestLineOf2(M);
        }
      }
    }
  change=true;
...
void mouseDragged() // executed when the mouse is dragged (while mouse buttom pressed)
  {
    if (startGame != 2) {
      if ((!keyPressed || (key=='a')|| (key=='i'))&&(!cutted)) P.dragPicked();   // drag selected point with mouse
      if (keyPressed) {
      ...
      if (keyPressed && key=='m') {
        if (moveCount == 0) {
          savePieces("data/ghost");
          pts temp = new pts();
          temp.declare();
          temp.loadPts("data/orig");
          R_whatever.add(temp);
        }
        D = Mouse();
        for(pts poly : R) {
          if (poly.pieceSelected(C, D)) {
            P = poly;
          }
        }
        P.dragAll();
...
String title ="Split Polygon Puzzle",            name ="Student: Congyao Zheng",
       subtitle = "  code for P2 for Jarek Rossignac's CS3451 class in the Fall 2016",
       
       menu="?:(show/hide) help, ~/!/@:snap pdf/jpg/fif, `:(start/stop) recording, A:save all, Q: quit",
       guide="click&drag at vertices: change shape, m&click&drag: move, r&drag: rotate, z&drag: scale,", // help info
       guide2 = " d&click: delete, i&click&drag: insert, s&click&drag: split";
       
String playMenu = "Guide [ l: load puzzle, e: escape, r: redo, q&click: select edges, f: deselect edges ]",
       Instruction = "hint: Click on the edges you think are matched to see what will happen! If anything wrong, use REDO!";