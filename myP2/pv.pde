...
pt pointOnArcThrough (pt A, pt B, pt C, float t) 
   ...
   boolean LineStabsEdge(pt A, pt B, pt C, pt D)
    {
      ...
      float res =  dot(U(A, B), U(C, D));
      boolean cross = det(U(A, B), U(A, C)) * det(U(A, B), U(A, D )) < 0;
      
      return res < 0.96 && cross;
    }
   ...
   boolean collide(pt A, pt B, pt C, pt D) {
     vec AB = V(A, B), AC = V(A, C), AD = V(A, D);
     vec CD = V(C, D), CA = V(C, A), CB = V(C, B);
